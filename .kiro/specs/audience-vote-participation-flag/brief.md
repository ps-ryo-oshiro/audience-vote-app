...
> **2026-09-10 改訂の注記**: 本 brief は初版時点のもの。以下の点は `requirements.md`（改訂履歴）と `design.md` の内容が優先する。
> - 接続先は投票アプリ専用の Firebase プロジェクト（`audience-vote-2026`）とし、審査アプリ（judge-app）のプロジェクトは読み取りのみで扱う
> - 認証は一切使わない（下記の「ハイブリッド案」「`signInAnonymously()`」は廃止）
> - 管理画面のCSV/Excel取込は廃止する。審査アプリの48チームを登録スクリプトで事前に一括登録し、紹介動画URLや名称の修正も同じ手順で追記する
> - 本戦確定12チームは常に表示し、管理画面では敗者復活候補だけを表示・非表示に切り替える。投票画面にはエントリーNoを表示する
> - 接続値は git に含めない。配信するファイルは `.assetsignore` の許可リストで絞る
> - 「新規ファイルは作らない」制約は、`.assetsignore`・`firebase-config.example.js`・`firebase.json`・`scripts/team-patch.mjs`・`docs/TEAM-DATA.md` に限って例外とする
